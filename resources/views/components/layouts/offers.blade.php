...
<section class="w-full py-16 lg:py-24"
    ...
    <x-layouts.container class="flex flex-col items-center gap-[57px]">
        <h2 class="text-center text-2xl tracking-tight text-white sm:text-3xl lg:text-[35px]">{{ $heading }}</h2>
        ...
        <div class="relative w-full">
                       ...
                       class="group relative block w-[686px] max-w-[88%] shrink-0 snap-start overflow-hidden rounded-2xl">
                        ...
                        <div class="absolute inset-x-0 bottom-0 flex flex-col gap-4 p-6 lg:p-9">
                            ...
                            <div class="flex flex-wrap items-center justify-between gap-2">
                                <p class="text-2xl font-semibold tracking-tight text-white lg:text-[40px]">{{ $room['name'] }}</p>
                                <p class="flex items-baseline gap-1 text-white">
                                    <span class="text-2xl font-bold tracking-tight lg:text-[30px]">{{ $room['price'] }}</span>
            ...
            <button type="button" @click="scrollByCard(-1)" aria-label="Previous"
                    class="absolute left-3 top-1/2 hidden size-12 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-[#222a1f] shadow-lg transition hover:bg-white lg:flex">
                <svg class="size-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m15 18-6-6 6-6" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
            <button type="button" @click="scrollByCard(1)" aria-label="Next"
                    class="absolute right-3 top-1/2 hidden size-12 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-[#222a1f] shadow-lg transition hover:bg-white lg:flex">
                <svg class="size-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m9 18 6-6-6-6" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
        </div>

        <a href="#"
           class="flex h-[73px] w-[341px] max-w-full items-center justify-center gap-1.5 rounded-[10px] bg-[#ba6d04] font-poppins text-[22px] font-semibold tracking-tight text-white transition hover:bg-[#a35f03]">
            Explore more
            <svg class="size-6 -rotate-30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </a>
    </x-layouts.container>
</section>
