<x-layouts.web title="Rooms & Apartment — Retiro Del Rocio"
    ...
    <section class="w-full pt-10 pb-12 lg:pt-12 lg:pb-16">
        <x-layouts.container class="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between lg:gap-10">
            <h1 class="text-4xl font-semibold tracking-tight text-white sm:text-5xl lg:whitespace-nowrap lg:text-h1 xl:text-display lg:leading-none">
                Rocio Loft Residence
            </h1>

            <div class="flex min-w-0 flex-col gap-4 lg:shrink-0 lg:items-end">
                <button type="button" class="flex items-center gap-2 self-start text-body-lg font-medium tracking-tight text-white lg:self-end">
                    Filter search
                    <svg class="icon-md shrink-0" viewBox="0 0 27 27" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-miterlimit="10">
                ...
                <div class="no-scrollbar flex max-w-full gap-[14px] overflow-x-auto lg:justify-end">
                    @foreach ($filters as $filter)
                        <button type="button" @class([
                            'flex h-[58px] shrink-0 items-center justify-center whitespace-nowrap rounded-[8px] border-[0.5px] border-black/20 px-[26px] text-body font-bold tracking-tight text-black transition lg:text-body-lg',
